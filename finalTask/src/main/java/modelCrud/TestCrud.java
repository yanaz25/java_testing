...
public class TestCrud extends Crud {
    ...
    private static final String LATEST_TEST_QUERY = " AS t1 INNER JOIN (SELECT name," +
            " MAX(start_time) AS start_time FROM test WHERE %s = ? GROUP BY name) AS t2 " +
            "ON t1.name = t2.name AND t1.start_time = t2.start_time ORDER BY t1.start_time desc LIMIT ?;";
    ...
    public List<TestModel> getLatestTestsByProject(int projectId, int limit) {
        String query = String.format(LATEST_TEST_QUERY, PROJECT_ID_COLUMN);
        return selectAllByCondition(query, projectId, limit);
    }
